...
function renderTables(list) {
  ...
  list.forEach((table) => {
    const statusClass = statusMap[table.status] || "";
        ...
        <div class="card-content">
            ...
            <div class="extra-info ${
              table.status !== "Đang dùng" ? "hidden-info" : ""
                ...
                <span><i class="fa-solid fa-users"></i> Khách: ${
                  table.customers ?? 0
                }</span>

                <span><i class="fa-solid fa-utensils"></i> Món: ${
                  table.items ?? 0
                }</span>
...
window.initTable = function () {
  ...
  if (statusEl) statusEl.textContent = "Tất cả";
...
window.handleNewTable = (newTable) => {
  ...
  const nextId = window.getNextTableId();
  ...
  const tableToAdd = {
    ...
    name: newTable.name || `Bàn ${String(nextId).padStart(2, "0")}`,
